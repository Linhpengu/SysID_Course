...
refResults: dict = np.load("exercise7_task2_refSol.npz")
...
def checkValueSimilar(variable,
    ...
    # check values
    same_values_as_reference = np.all(
        np.isclose(variable, reference_variable, equal_nan=True, atol=tolerance))  # to avoid long output
...
def checkVariableSimilar(varName: str,
    ...
    return checkValueSimilar(variable, reference_variable, varName, tolerance=tolerance, absoluteValue=absoluteValue,
...
def test_B1():
    ...
    checkVariableSimilar("Phi_phy")


def test_B2():
    """ check if values are correct (0.5 points) """
    checkVariableSimilar("theta_phy")

def test_B3():
    """ check if values are correct (0.5 points) """
    checkVariableSimilar("covmatrix_estimate")

def test_B4():
    """ check if values are correct (0.5 points) """
    checkVariableSimilar("covmatrix_y")
    checkVariableSimilar("stds_y")
...
def test_E():
    """ check if values are correct (0.5 points) """
    checkVariableSimilar("Phi_ARX")
    checkVariableSimilar("theta_ARX")
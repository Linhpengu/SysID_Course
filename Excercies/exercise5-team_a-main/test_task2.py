...
def test_D1():
    """  """
    """ check if the functions are implemented correct (1 point) """
    # check if the objective function is implemented correctly (0.5 points)
    theta_test1 = refResults["theta_test1"]
    theta_test2 = refResults["theta_test2"]
    objectiveFunction_eval1 = refResults["objectiveFunction_eval1"]
    objectiveFunction_eval2 = refResults["objectiveFunction_eval2"]

    checkValueSimilar(studentScript.objectiveFunction(theta_test1),objectiveFunction_eval1,"Eval_ObjectiveFunction")
    checkValueSimilar(studentScript.objectiveFunction(theta_test2),objectiveFunction_eval2,"Eval_ObjectiveFunction")

    # check if the weibull pdf function is implemented correctly (0.5 points)
    k_test = refResults["k_test"]
    lambda_test = refResults["lambda_test"]
    v_test1 = refResults["v_test1"]
    v_test2 = refResults["v_test2"]
    weibullFunction_eval1 = refResults["weibullFunction_eval1"]
    weibullFunction_eval2 = refResults["weibullFunction_eval2"]

    # test the implementation of the weibull function with different values (compare results)
    checkValueSimilar(studentScript.weibullPDF(v_test1,k_test,lambda_test),weibullFunction_eval1,"Eval_WeibullFunction")
    checkValueSimilar(studentScript.weibullPDF(v_test2,k_test,lambda_test),weibullFunction_eval2,"Eval_WeibullFunction")


def test_D2():
    """ check if the minimization results are correct (1 points) """
    checkVariableSimilar("thetaStar",tolerance=1e-4)
...
def test_E1():
    ...
    checkVariableSimilar("windSpeedProbability")

def test_E2():
    """ check if values are correct (1 point) """
    checkVariableSimilar("expectedPower")